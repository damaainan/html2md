...
def getJsonFromApi(url, param):
    ...
    headers = {
        ...
        'sec-ch-ua-mobile': '?0',
        'user-agent':
        'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.141 Safari/537.36',
    ...
    if data['data'] != None and len(data['data']) > 0:
        for j in range(len(data['data'])):
            ...
            ret.append({
                ...
                data['data'][j]['article_info']['article_id'],
                "title":
                data['data'][j]['article_info']['title'],
                "msgid":
                data['data'][j]['article_info']['article_id'],
                "user":
                ...
                "updated":
                data['data'][j]['article_info']['mtime']
            })
    ...
    if len(data['data']['article_list']) > 0:
        ...
        for j in range(len(dlist)):
            ...
            ret.append({
                "url":
                "https://juejin.cn/post/" +
                dlist[j]['article_info']['article_id'],
                "title":
                dlist[j]['article_info']['title'],
                "msgid":
                dlist[j]['article_info']['article_id'],
                "archive":
                tag_user,
                "user":
                dlist[j]['author_user_info']['user_name'],
                "type":
                tag,
                "created":
                dlist[j]['article_info']['ctime'],
                "updated":
                dlist[j]['article_info']['mtime'],
                "co_folder_pre":
                tag_user + "-" + tag
            })
...
def CleanResult(data, title):
    ...
    for i in range(len(data)):
        ...
        data[i]['archive'] = (data[i]['archive']
                              if data[i].__contains__('archive') else title)
