...
REALPATH = os.path.dirname(
    os.path.dirname(os.path.dirname(
        os.path.abspath(__file__)))) + '/out/juejin'
...
class JuejinGenPdf():
    ...
    def deal(self, url, title, path):
        ...
        if len(title_img) > 0:
            ...
            title_img_str = title_img_str.decode().replace(
                title_img_str.get("data-src"), nsrc)
            # sys.exit(0)
        ...
        for im in range(len(imgs)):
            ...
            if imgs[im].get('src'):
                ...
                if imgs[im].get('data-src'):
                    ...
                    # src=imgs[im]['src']
                    continue
                if src.find('githubusercontent.com') > -1 or src.find(
                        'github.com') > -1 or src.find('data:image') > -1:
                    continue
...
'''
        ...
        for csi in range(len(css_inner)):
            ...
            # print(type(css[cs].get_text()))
            # font = font + "<style>" + css[cs].get_text() + "</style>"
            font = font + "<style>" + HTMLParser().unescape(
                html.tostring(css_inner[csi]).decode()) + "</style>"
        ...
        '''
        fhtml = font + '</head><body style="margin:20px 80px;">' + str(
            title_img_str) + str(
        ...
        for src in imgDict:
            if src != "":
                ...
                fhtml = fhtml.replace(
                    imgDict[src], "<img style='max-width:100%;' src='./pic/" +
                    newsrc + "'/>")
    ...
    def dealAns(self, url, title, path):
        ...
        for im in range(len(imgs)):
            ...
            if imgs[im].has_attr('src'):
                ...
                if imgs[im].has_attr('class'):
                    if "ztext-gif" in imgs[im].get('class'):
                        src = src.replace('.jpg', '.webp').replace(
                            '.png', '.webp').replace('.jpeg', '.webp')
                imgDict[src] = imgs[im].decode()
        # print(imgDict)
        # return
        ...
        fhtml = font + '</head><body style="margin:20px 80px;">' + str(
            fheader) + str(fhtml) + '</body></html>'
        print(title)
        # 增大较小的字体
        # html=html.replace("font-size: 14px","font-size: 18px").replace("font-size: 12px","font-size: 18px").replace("font-size: 11px","font-size: 16px").replace("font-size: 11.9px","font-size: 16px")

        fhtml = re.sub(r"font-size: 1[0-5]\.{0,1}[0-9]{0,1}[0-9]{0,1}px;",
                       'font-size: 16px;', fhtml)
    ...
    def getHTMLText(self, url):
        ...
        driver = webdriver.Chrome(executable_path=path_config,
    ...
    def dealHtml(self, url, title, path):
        ...
        htmlstr = re.sub(r'height: [0-9]{1,4}\.{0,1}[0-9]{0,17}px !important;',
                         '', htmlstr)
        htmlstr = re.sub(r'width: [0-9]{1,4}\.{0,1}[0-9]{0,17}px !important;',
                         '', htmlstr)
...
'''
        ...
        for cs in range(len(css)):
            # font = font + "<style>" + css[cs].get_text() + "</style>"
            font = font + "<style>" + HTMLParser().unescape(
                html.tostring(css[cs]).decode()) + "</style>"
            # HTMLParser().unescape(str1.decode())
        ...
        rpath = os.path.dirname(os.path.dirname(
        ...
        pfile = pdf_path + ntitle + '.pdf'
        os.system(
            'wkhtmltopdf --dpi 300 --enable-plugins --enable-forms "{}" "{}"'.
            format(hfile, pfile))
