...
class ZhihuStoreData():
    ...
    def getAblumListFromSql(self, url):
        sql = simpleToolSql("url")
        # res = sql.query("select * from zh_article where state=0;")
        res = sql.query("select * from zh_ablum where url='{u}' and update_at<'{t}';".format(u=url,t=(datetime.datetime.now()-datetime.timedelta(hours=2)).strftime("%Y-%m-%d %H:%M:%S")))
        # print(res)
        sql.close()
        return res
    ...
    def getAblumListFromAuthorAndTitle(self, title):
        sql = simpleToolSql("url")
        # res = sql.query("select * from zh_article where state=0;")
        res = sql.query("select * from zh_ablum where title='{ti}' and update_at<'{t}';".format(ti=title,t=(datetime.datetime.now()-datetime.timedelta(hours=2)).strftime("%Y-%m-%d %H:%M:%S")))
        # print(res)
        sql.close()
        return res
    def updateAblum(self, id):
        ...
        # res = sql.query("select * from zh_article where state=0;")
        # print("-*-*-*-*")
        res = sql.execute("update zh_ablum set update_at='{t}' where id={id};".format(t=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),id=id))
        print("更新===",res)
        sql.close()
        return res
    ...
    def getAblums(self):
        sql = simpleToolSql("url")
        res = sql.query("select * from zh_ablum where update_at<'{t}';".format(t=(datetime.datetime.now()-datetime.timedelta(hours=2)).strftime("%Y-%m-%d %H:%M:%S")))
        # print(res)
        sql.close()
        return res
