...
def getJsonData(oldurl: str, msgid: int):
    ...
    if "article_list" in data['getalbum_resp'].keys():
        ...
        if isinstance(art, dict):
            link.append(
                {"link": art['url'], "title": art['title'], "msgid": art['msgid']})
            msgid.append(art['msgid'])
        elif isinstance(art, list):
            for val in art:
                link.append(
                    {"link": val['url'], "title": val['title'], "msgid": val['msgid']})
...
def getHomeJsonData(oldurl: str, mount: int):
    ...
    if len(data['appmsg_list']) > 0:
        ...
        if isinstance(art, dict):
            link.append(
                {"link": art['link'], "title": art['title'], "msgid": art['appmsgid']})
            msgid.append(art['appmsgid'])
        elif isinstance(art, list):
            for val in art:
                link.append(
                    {"link": val['link'], "title": val['title'], "msgid": val['appmsgid']})
